...
from aflow import aflow, async_exec, flog
from aflow.f import impure, breaker, lift, spread, identity, F, F1
...
class TestAsyncFlow(unittest.IsolatedAsyncioTestCase):
    ...
    impure_flow = aflow.from_flow(simple_flow) >> impure(print_some_stuff, print_some_stuff)
    ...
    async def test_flow_exec_parallel(self):
        r1, r2 = await (async_exec(TestAsyncFlow.simple_flow) | TestAsyncFlow.impure_flow).run()
    ...
    async def test_impure_from_instance(self):
        test = Test()
        flow = aflow.from_flow(TestAsyncFlow.simple_flow) >> impure(test.print_some_stuff)
        await flow.run()
    ...
    async def test_lift(self):
        ...
        flow = (
            aflow.from_flow(TestAsyncFlow.simple_flow)
            >> lift(test.return_new_value, x=12)
        )
    ...
    async def test_F1(self):
        ...
        flow = (
                aflow.from_args(Test())
                >> F1(attr, extractor=attrgetter("attribute"))
        )
        result = await flow.run()
